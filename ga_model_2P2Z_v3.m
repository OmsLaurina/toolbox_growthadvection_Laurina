function varargout=ga_model_2P2Z_v3(Nsupply,C_nut,varargin)
...
[arg,flag]=ga_read_varargin(varargin,[{'nbdays_advec',20,'dt',0.1,'time',[],'upw_duration',1},default_parameters],{'plot'});
...
Nnew(1)=C_nut;%0.3;%0.25E-3; 
Nreg(1)=C_nut;%0.3;%0.25E-3; 
...
for t=2:nb_time
    ...
    %%%% MODIFS
    u_small_Nnew(t-1)= Nnew(t-1)/(arg.kNnew+Nnew(t-1))*arg.umax_small;
    u_small_Nreg(t-1)= Nreg(t-1)/(arg.kNreg+Nreg(t-1))*arg.umax_small;
...
output=struct('units',units,'time',time,'Nsupply',Nsupply,...
	...
	'u_small_Nreg',u_small_Nreg,'u_small_Nnew',u_small_Nnew,'u_big',u_big,'g_small',g_small,'g_big',g_big,'attributs',struct('arg',arg));
...
%     xlim([min(output.time) max(output.time)]);
    ...
    f_monod(N_theo,arg.umax_small,arg.kNreg);
    f_monod(N_theo,arg.umax_small,arg.kNnew);
    f_monod(N_theo,arg.umax_big,arg.kNnew);
    
    f_monod(output.Nreg,arg.umax_small,arg.kNreg);
