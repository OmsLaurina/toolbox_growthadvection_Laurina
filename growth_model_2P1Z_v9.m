...
%       Add conversion factor : prey to predator
...
if flag.plot
    ...
    plot(output.PO4, output.P1)
    xlabel('PO4')
    ylabel('P1')
    % Tracé du champ de vecteur sur le portrait de phase
    quiver(output.PO4, output.P1, dpo4_dt, dp1_dt)
    ...
    plot(output.PO4, output.P2)
    xlabel('PO4')
    ylabel('P2')
    quiver(output.PO4, output.P2, dpo4_dt, dp2_dt)
    
    % 3. P1 vs Z
    figure, hold on
    grid on 
    box on
    dZ_dt = gradient(output.Z, arg.dt);
    dp1_dt = gradient(output.P1, arg.dt);
    plot(output.Z, output.P1)
    xlabel('Z')
    ylabel('P1')
    quiver(output.Z, output.P1, dZ_dt, dp1_dt)
    ...
    dZ_dt = gradient(output.Z, arg.dt);
    dp2_dt = gradient(output.P2, arg.dt);
    plot(output.Z, output.P2)
    xlabel('Z')
    ylabel('P2')
    quiver(output.Z, output.P2, dZ_dt, dp2_dt)
