function varargout=ga_model_2P2Z_fromPsupply_SST_Biss_Med(Nsupply,SSTsupply,Nini,psmall_ini,pbig_ini,varargin)
...
for t=2:nb_time
	...
	u_small(t-1)=Nreg(t-1)/(arg.kNreg+Nreg(t-1))*0.81*exp(0.0631*SSTsupply(t-1));
...
figure('DefaultAxesFontSize',22), hold on
	plot(datenum(1950,01,01)+output.time,output.P_small,'LineWidth',2)
	plot(datenum(1950,01,01)+output.time,output.P_big,'LineWidth',2)
	plot(datenum(1950,01,01)+output.time,output.Z_small,'LineWidth',2)
	plot(datenum(1950,01,01)+output.time,output.Z_big,'LineWidth',2)
	ylabel(output.units.P_small)
	datetick('x','dd-mmm-yyyy','keeplimits')
    if min(output.time)>datenum(1950,1,1), datetick('x','keeplimits'), xlabel('Time')
	else,datetick('x','dd-mmm-yyyy','keeplimits'), xlabel('Time (days)')
	end
	legend({'P\_small','P\_big','Z\_small','Z\_big'},'Location','best');
	title('Sortie du modèle (concentration planctonique en fonction du temps)')
