function varargout=ga_model_2P2Z_v4(Nsupply,C_nut,varargin)
...
for t=2:nb_time
    ...
    %%%% MODIFS
	Nreg(t)=Nreg(t-1)+regeneration_Nreg(t)*arg.dt-PP_small1(t)*arg.dt;  Nreg(Nreg<=0)=0;
    Nnew(t)=Nnew(t-1)+Nsupply(t)*arg.dt-PP_big(t)*arg.dt-PP_small2(t)*arg.dt; Nnew(Nnew<=0)=0;
    %%%% MODIFS
...
output=struct('units',units,'time',time,'Nsupply',Nsupply,...
	...
	'Chl',P_small*12/arg.cChl_small+P_big*12./arg.cChl_big,'PP',(PP_big+PP_small1+PP_small2)*1E-3*12*365.25,...
...
%     %Monod equation
%     figure, hold on
%     N_theo_Nnew = linspace(0,50,length(arg.time));
%     N_theo_Nreg = linspace(0,50,length(arg.time));
%     f_monod(N_theo_Nnew,N_theo_Nreg,arg.umax_small,arg.kNreg,2);
%     f_monod(N_theo_Nnew,N_theo_Nreg*0,arg.umax_big,arg.kNnew,1);
%     f_monod(output.Nnew,output.Nreg,arg.umax_small,arg.kNreg,2);
%     f_monod(output.Nnew,output.Nreg*0,arg.umax_big,arg.kNnew,1);
%     xlabel('[PO4] mmolC.m³')
%     ylabel('\mu d^{-1}');
