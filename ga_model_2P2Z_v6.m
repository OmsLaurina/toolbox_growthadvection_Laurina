function varargout=ga_model_2P2Z_v6(Nsupply,C_nut,varargin)
...
default_parameters={...
'umax_small',1.9872,'umax_big',2.7648,'gmax_small',1.4226,'gmax_big',1.4256,...%1.4256,...	% maximum growth and grazing rates (d^{-1}) (Baklouti et al., 2021)
...
%%%% MODIFS
PP_big1=time*NaN; 			% primary production for P_big (mmolC m^{-3} d^{-1})
PP_big2=time*NaN; 			% primary production for P_big (mmolC m^{-3} d^{-1})
...
for t=2:nb_time
    ...
    %%%% MODIFS
    u_big1=Nreg(t-1)/(arg.kNreg+Nreg(t-1)+Nnew(t-1))*arg.umax_big;
    u_big2=Nnew(t-1)/(arg.kNnew+Nreg(t-1)+Nnew(t-1))*arg.umax_big;
    u_big(t-1)=u_big1+u_big2;
    ...
    %%%% MODIFS
    PP_big1(t)=u_big1*P_big(t-1); 
    PP_big2(t)=u_big2*P_big(t-1); 
    ...
    %%%% MODIFS
    maxPP_big1=(Nreg(t-1)+regeneration_Nreg(t)*arg.dt)/arg.dt;
    maxPP_big2=(Nnew(t-1)+Nsupply(t)*arg.dt)/arg.dt;
    ...
    %%%% MODIFS
    if PP_big1(t)>maxPP_big1, PP_big1(t)=maxPP_big1; end
    if PP_big2(t)>maxPP_big2, PP_big2(t)=maxPP_big2; end
    ...
    %%%% MODIFS
	Nreg(t)=Nreg(t-1)+regeneration_Nreg(t)*arg.dt-PP_small1(t)*arg.dt-PP_big1(t)*arg.dt; Nreg(Nreg<=0)=0;
    Nnew(t)=Nnew(t-1)+Nsupply(t)*arg.dt-PP_big2(t)*arg.dt-PP_small2(t)*arg.dt; Nnew(Nnew<=0)=0;
    %%%% MODIFS
    
	P_big(t)=P_big(t-1)+PP_big1(t)*arg.dt+PP_big2(t)*arg.dt-G_big1(t)*arg.dt-death_Pbig(t)*arg.dt ;P_big(P_big<=0)=0;
...
output=struct('units',units,'time',time,'Nsupply',Nsupply,...
	...
	'Chl',P_small*12/arg.cChl_small+P_big*12./arg.cChl_big,'PP',(PP_big1+PP_big2+PP_small1+PP_small2)*1E-3*12*365.25,...
	'u_small',u_small,'u_big',u_big,'g_small',g_small,'g_big',g_big, 'PP_small2', PP_small2, 'PP_big1', PP_big1, 'PP_big2', PP_big2,'attributs',struct('arg',arg));
...
if flag.plot
    ...
    %Monod equation
    figure, hold on
    N_theo_Nnew = linspace(0,10,length(arg.time));
    N_theo_Nreg = linspace(0,10,length(arg.time));
    f_monod(N_theo_Nnew,N_theo_Nreg,arg.umax_small,arg.kNreg,2);
    f_monod(N_theo_Nnew,N_theo_Nreg,arg.umax_big,arg.kNnew,2);
    f_monod(output.Nnew,output.Nreg,arg.umax_small,arg.kNreg,2);
    f_monod(output.Nnew,output.Nreg,arg.umax_big,arg.kNnew,2);
    xlabel('[PO4] mmolC.m³')
    ylabel('\mu d^{-1}');
    legend({'P\_small\_theo','P\_big\_theo','P\_small\_mod','P\_big\_mod' })
    
%     %Nnew fraction in Psmall and in Pbig
%     Tot_Nnew = (output.Nnew./(output.Nreg+output.Nnew+arg.kNreg)*arg.umax_small)+(output.Nnew./(output.Nnew+arg.kNnew)*arg.umax_big);
%     F_Nnew_small = (output.Nnew./(output.Nreg+output.Nnew+arg.kNreg)*arg.umax_small)./Tot_Nnew;
%     F_Nnew_big = (output.Nnew./(output.Nnew+arg.kNnew)*arg.umax_big)./Tot_Nnew;
%     
%     figure, hold on
%     plot(output.time, F_Nnew_small*100,'LineWidth',2)
%     plot(output.time, F_Nnew_big*100,'LineWidth',2)
%     xlabel('Time (days)')
%     ylabel('(u\_Nnew)/(Nnew\_tot) (%)');
%     legend('F\_Nnew\_small', 'F\_Nnew\_big')
    
%     %Nnew quantity in Psmall and in Pbig
%     Q_Nnew_small = (output.Nnew./(output.Nreg+output.Nnew+arg.kNreg)*arg.umax_small);
%     Q_Nnew_big = (output.Nnew./(output.Nnew+arg.kNnew)*arg.umax_big);
%     
%     figure, hold on
%     plot(output.time, Q_Nnew_small,'LineWidth',2)
%     plot(output.time, Q_Nnew_big,'LineWidth',2)
%     xlabel('Time (days)')
%     ylabel('(u\_Nnew)/(Nnew\_tot) (%)');
%     legend('F\_Nnew\_small', 'F\_Nnew\_big')
%     
%     %Nnew/Nreg fraction in Psmall
%     Tot_N= output.u_small;
%     F_Nnew = (output.Nnew./(output.Nreg+output.Nnew+arg.kNnew)*arg.umax_small)./Tot_N;
%     F_Nreg = (output.Nreg./(output.Nreg+output.Nnew+arg.kNreg)*arg.umax_small)./Tot_N;
%     
%     figure, hold on
%     plot(output.time, F_Nnew*100,'LineWidth',2)
%     plot(output.time, F_Nreg*100,'LineWidth',2)
%     xlabel('Time (days)')
%     ylabel('(u\_N)/(N\_tot) (%)');
%     legend('F\_Nnew', 'F\_Nreg')
%     
