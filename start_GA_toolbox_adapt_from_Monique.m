...
%addpath('Documents/toolbox_GrowthAdvection_Monique/') in command window first

% addpath('Ariane/'); %path where Ariane is installed
% addpath('Documents/toolbox_GrowthAdvection_Monique/utils/'); %needed functions
% addpath('Documents/toolbox_GrowthAdvection_Monique/inputs/'); %needed inputs
% addpath('Documents/toolbox_GrowthAdvection_Monique/outputs/'); %needed outputs
...
output2=ga_model_2P2Z_v7(Psupply,0.5,0.6,0.1,'kP_small',1,'kP_big',2,'gmax_small',1.4226,'gmax_big',1.1226,'time',time,'plot');
...
name_curr='PROTEVS_bio2';	% using currents into Ariane_workplace/currents_data
...
%print('-djpeg','-r300','Documents/toolbox_GrowthAdvection/outputs/GArun_start_20080501.jpg')
...
% 
% %to get the variable field wanted
% values2D = getfield(zoo2D,var_model);
% units2D = getfield(zoo2D.units,var_model);
% 
% figure, hold on
% pcolor(zoo2D.lon-0.125/2,zoo2D.lat-0.125/2,values2D(:,:,itime)), shading flat
% plot(coast_x,coast_y,'k')
% xlim([-125.5 -120]), ylim([34 38])
% hbar=colorbar; caxis(caxis_lim)
% set(get(hbar,'title'),'string',units2D);
% xlabel('Longitude'), ylabel('Latitude'), title([var_model, ' mapped output, May 2008'], 'Interpreter', 'none')
