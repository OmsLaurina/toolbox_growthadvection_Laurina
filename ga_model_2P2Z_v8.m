function varargout=ga_model_2P2Z_v8(Nsupply,C_nut,varargin)
...
default_parameters={...
'umax_small',1.9872,'umax_big',2.7648,'gmax',1.4226,...% maximum growth and grazing rates (d^{-1}) (Baklouti et al., 2021)
...
'P_small_ini',0.6,'P_big_ini',0.1,'Z_ini',0.3};	% initial biomass (mmolC m^{-3}) (In situ for P_small_ini with method from Marrec et al., 2018 and Tzortzis et al., 2021 & further)
...
for t=2:nb_time
    ...
    %%%% MODIFS
    g1=P_big(t-1)/(arg.kG+P_small(t-1)+P_big(t-1))*arg.gmax; 	
	g2=P_small(t-1)/(arg.kG+P_small(t-1)+P_big(t-1))*arg.gmax;
	g(t-1)=g1+g2;
	%%%% MODIFS
    ...
    %%%% MODIFS
    P_big(t)=P_big(t-1)+PP_big(t)*arg.dt-G1(t)*arg.dt-death_Pbig(t)*arg.dt ;P_big(P_big<=0)=0;
...
units=struct('time','days','Nsupply','mmolC m^{-3} d^{-1}',...
	...
	'u_small','d^{-1}','u_big','d^{-1}','g','d^{-1}', 'PP_small', 'gC m^{-3}/yr', 'PP_big','gC m^{-3}/yr');
output=struct('units',units,'time',time,'Nsupply',Nsupply,...
	...
	'u_small',u_small,'u_big',u_big,'g',g, 'PP_small', PP_small,'PP_big', PP_big, 'attributs',struct('arg',arg));
...
if flag.plot
	...
	plot(output.time,output.Z,'LineWidth',2,'Color','#4DBEEE')
