function varargout=ga_model_2P2Z_fromPsupply_SST_Biss_Med_PPsmall_fct_Nnew(Nsupply,SSTsupply,Nini,psmall_ini,pbig_ini,varargin)
...
'P_small_ini',5*psmall_ini,'P_big_ini',pbig_ini,'Z_small_ini',0.3,'Z_big_ini',0.2};	% initial biomass (mmolC m^{-3}) (In situ for P_small_ini with method from Marrec et al., 2018 and Tzortzis et al., 2021 & further)
...
%% -------------- SST
if length(SSTsupply)==1
    SSTsupply_max = SSTsupply;
    SSTsupply=zeros(nb_time,1);
    SSTsupply(time<arg.upw_duration)=SSTsupply_max;
end
...
%%%% MODIFS
u_small_Nnew=time*NaN; 			% P_small growth rate (Nreg-limited) (d^{-1})
u_small_Nreg=time*NaN; 			% P_small growth rate (Nreg-limited) (d^{-1})
...
%%%% MODIFS
PP_small_Nnew=time*NaN; 			% primary production for P_small (mmolC m^{-3} d^{-1})
PP_small_Nreg=time*NaN; 			% primary production for P_small (mmolC m^{-3} d^{-1})
...
for t=2:nb_time
	...
	%%%% MODIFS
 	u_small_Nnew(t-1)= (Nnew(t-1)/(arg.kNnew+Nnew(t-1)))*0.81*exp(0.0631*SSTsupply(t-1));
    u_small_Nreg(t-1)= (Nreg(t-1)/(arg.kNreg+Nreg(t-1)))*0.81*exp(0.0631*SSTsupply(t-1));
    u_small(t-1) = (u_small_Nnew(t-1)+u_small_Nreg(t-1));
    %%%% MODIFS
	...
	%%%% MODIFS
	PP_small_Nnew(t)=u_small_Nnew(t-1)*P_small(t-1); 
    PP_small_Nreg(t)=u_small_Nreg(t-1)*P_small(t-1);
	...
	maxPP_small=((Nreg(t-1)+regeneration_Nreg(t)*arg.dt)/arg.dt)+((Nnew(t-1)+Nsupply(t)*arg.dt)/arg.dt);
	...
	%%%% MODIFS
	Nnew(t)=Nnew(t-1)+Nsupply(t)*arg.dt-(PP_big(t)*arg.dt+PP_small_Nnew(t)*arg.dt); 
	Nreg(t)=Nreg(t-1)+regeneration_Nreg(t)*arg.dt-PP_small_Nreg(t)*arg.dt;
    %%%% MODIFS
