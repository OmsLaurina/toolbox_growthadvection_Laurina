function varargout=pcolor_gmax1_umax1_v9(Nsupply,C_nut,varargin)
...
P_1=zeros(length(l_gmax1),length(l_umax1));
P_2=zeros(length(l_gmax1),length(l_umax1));
...
for gmax1=l_gmax1
    ...
    for umax1=l_umax1
        ...
        for t=2:nb_time
            ...
            % functional response
            g1=P1(t-1)/(arg.kG+P1(t-1)+P2(t-1))*gmax1; 	
...
if flag.plot
    ...
    figure,hold on
    box on
    x = linspace(min_gmax1,max_gmax1,n);
    y = linspace(min_umax1,max_umax1,n);
    [X,Y] = meshgrid(x,y);
